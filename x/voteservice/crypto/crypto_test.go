...
func TestCreate1outof2ZKPYesVote(t *testing.T) {
	...
	y, a, b, a2, b2, res, err := Create1outof2ZKPYesVote(sender, vote1ZK.xG, vote1_yG, vote1ZK.W, vote1ZK.R, vote1ZK.D, vote1ZK.X)
	...
	assert.Equal(t, "87085173367699447715488437323496198203900388000487606358045377572991016207181", a.X.String())
	assert.Equal(t, "74042786953562842134126339010820053310954021259496628878671621002953616652989", a.Y.String())
	assert.Equal(t, "35363326994709962702584347792183188284211544509318631805915419782284264375762", b.X.String())
	assert.Equal(t, "52840274120571261927524975101728893660081259998744933965821274996904183655029", b.Y.String())
